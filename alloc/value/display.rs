...
impl FormatterRegistry {
    fn new() -> Self {
        Self { formatters: HashMap::new() }
    }

    fn register<T: 'static>(&mut self, formatter: Box<dyn ValueFormatter>) {
        self.formatters.insert(TypeId::of::<T>(), formatter);
    }

    pub fn get(&self, type_id: TypeId) -> Option<&dyn ValueFormatter> {
        self.formatters.get(&type_id).map(|boxed| boxed.as_ref())
    }
}
...
pub fn init_registry() -> FormatterRegistry {
    ...
    registry.register::<List<Value>>(Box::new(SequenceFormatter::new("[", "]")));
...
impl ValueFormatter for NullFormatter {
    fn format(&self, _: &dyn std::any::Any, f: &mut Formatter) -> fmt::Result {
        f.write_str("null")
    }
}
...
impl ValueFormatter for SequenceFormatter {
    fn format(&self, value: &dyn std::any::Any, f: &mut Formatter) -> fmt::Result {
        ...
        if let Some(list) = value.downcast_ref::<List<Value>>() {
