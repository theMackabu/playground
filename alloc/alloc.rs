...
impl<T> RawList<T> {
    fn new() -> Self {
        ...
        RawList { ptr: NonNull::dangling(), cap }
    }
...
impl<T> List<T> {
    ...
    pub fn new() -> Self {
        List { buf: RawList::new(), len: 0 }
    }

    pub fn from_slice(slice: &[T]) -> List<&T> {
        let mut list = List::new();

        slice.iter().enumerate().for_each(|(idx, elem)| list.insert(idx, elem));
        list
    }
...
macro_rules! list {
    () => ($crate::list::List::new());
    ($($x:expr),* $(,)?) => ({
        const LEN: usize = 0 $(+ { let _ = $x; 1 })*;
        $crate::alloc::List::from_slice(&[$($x),*][..LEN])
    });
}
